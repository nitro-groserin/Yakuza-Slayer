...
// 2曲目のaudioElementにのみloopプロパティを設定
if (audioElements[1]) { // 2曲目が存在するか確認
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function showPre(index) {
    ...
    if (div2) div2.style.display = (index >= 2 && index < 100) || (index > 100  && index <= 100) || (index > 100) ? 'block' : 'none';
    if (div5) div5.style.display = (index >= 54 && index < 54) || (index > 55  && index <= 55) || (index > 100) ? 'block' : 'none';
    if (div7) div7.style.display = index >= 100 && index < 100 ? 'block' : 'none';
    if (div8) div8.style.display = index >= 79 && index < 92 ? 'block' : 'none';
    if (div9) div9.style.display = index >= 0 && index < 100 ? 'block' : 'none';
